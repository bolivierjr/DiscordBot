...
from discord_bot.models.users import User, UserSchema
from discord_bot.utils.errors import LocationNotFound, WeatherNotFound
from discord_bot.utils.services import WeatherService
from discord_bot.utils.users import AnonymousUser, get_user
from discord_bot.utils.weather import OpenWeatherMapAPI, WeatherAPI
...
def get_mock_user():
    return User(nick="Johnno", location="New York", region="New York", coordinates="40.714,-74.006", format=1,)
...
##################################
class UtilsGetUserTestCase(TestCase):
    def tearDown(self):
        User.drop_table()
        test_db.close()
    ...
    def test_get_users(self):
        ...
        User.create_table()
        User.create(
            nick="Johnno", location="New Orleans", region="Louisiana", coordinates="29.974,-90.087", format=1,
        )
...
##################################
class UtilsWeatherServiceTestCase(TestCase):
...
##################################
class UtilsOpenWeatherMapApiTestCase(TestCase):
...
# @mock.patch("requests.get", autospec=True)
# class UtilsDisplayFormatTestCase(TestCase):
...
class UtilsFormatDirectionTestCase(TestCase):
...
class UserModelTestCase(TestCase):
    ...
    def test_user_model_nick_unique(self) -> None:
        ...
        """
        user = User(nick="Johnno", location="Covington", region="Louisiana", coordinates="29.974,-91.087", format=1,)

        with self.assertRaises(DatabaseError):
            user.save()


class UserModelCreateTables(TestCase):
    def tearDown(self):
        User.drop_table()
        test_db.close()
